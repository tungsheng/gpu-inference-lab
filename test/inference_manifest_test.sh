...
assert_contains "${DASHBOARD_CONTENT}" 'max by (profile, policy, target)' "the experiment dashboard should group series by profile, policy, and HPA target"
assert_contains "${DASHBOARD_CONTENT}" '{{profile}}/{{policy}}/{{target}}' "the experiment dashboard legend should distinguish targets inside the same policy"
...
assert_contains "${SCRIPT_CONTENT}" '/metrics/job/gpu-serving-measure/profile/${EVALUATION_PROFILE}/policy/${CURRENT_POLICY}/target/${CURRENT_HPA_TARGET_AVERAGE_VALUE}' "the evaluation flow should push summary metrics with policy and target labels"
